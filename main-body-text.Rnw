...
The MSP model was run on the 2000 simulations to test its effectiveness at identifying contaminated samples and estimating the contamination rate. As with the SSP model, the hyperparameters $\alpha$, $\beta$, and $\theta_{\ell}$ ($\lambda$) were set to 0.5 during the experiments.
...
\chapter{Conclusion}
This paper presented a model for identifying contaminated samples genotyped using single nucleotide polymorphisms (SNPs).  Unlike highly polymorphic microsatellites, SNPs only have two alleles; therefore, contaminated samples can not be dected by simply identifying samples that are genotyped with more than one allele at a specific locus.  Models have been developed to identify contaminated samples of human DNA genotyped with SNPs, but few studies have investigated ways to detect contaminated samples of DNA from non-model organisms, like salmon, and these studies have used a fixed rate of contamination for inference.  The Baysian models developed in this study identify contaminated samples given an unknown rate of contamination.  Two different models were developed in this study: (1) the SSP model, used for estimating allele frequencies of a single population and (2) the MSP model, used for the population identification of organisms sampled from a mixture of multiple populatins.  Both models were developed assuming that populations were not in linkage disequilibrium, that each sample's contamination status is idendepent of those of other samples, and that the DNA that contaminates a sample is drawn randomly from the population.  The performance of both models were tested using simulations with collections \emph{in silico} data.

The results of the simulations showed that both the SSP and MSP models were able to accurately estimate the contaminateion rate, $\rho$, and correctly identify the contamination status of the vast majority of the samples.  In addition the SSP model is also able to effectively estimate the allele frequencies of the population from which the collection of samples originated.  Notably, the SSP model was most effective with 60 and above loci, and was not able to accurately identify the contamination status with samples genotyped with 20 loci.

The Bayesian model described in this paper assumes that a contaminated sample includes two individuals and that the genotypes of both individuals will be registered at every loci.  Ideal use of the models. Equal mixing of contamination, lab contamination.

Further exploration: percent of loci contaminated, will small contamination be read, best for mixing of two samples in lab setting