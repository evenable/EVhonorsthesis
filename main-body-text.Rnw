...
Bayesian inference combines new evidence with prior knowledge to determine the sequence of events that most likely led to a given outcome.  In Bayesian inference, updating prior beliefs using newly aquired evidence is done using Bayes' rule, which is given by Equation ~ref{bayes}.  The variable $\theta$ represents an unknown, like the allele frequencies at a set of loci, and the variable $y$ is the known paramenter.  Baye's 
